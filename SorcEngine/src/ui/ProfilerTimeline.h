...
#define RGBA_LE(col) (((col & 0xff000000) >> (3 * 8)) + ((col & 0x00ff0000) >> (1 * 8)) + ((col & 0x0000ff00) << (1 * 8)) + ((col & 0x000000ff) << (3 * 8)))
    ...
    public:
        ...
        std::vector<float> frameTimeHistory; // ms

        ProfilerTimeline(size_t framesCount)
        {
            ...
            fpsHistory.reserve(framesCount);
            frameTimeHistory.reserve(framesCount);
        }
        ...
        void PushPerf(float fps, float frameTimeMs)
        {
            if (fpsHistory.size() >= frames.size()) { fpsHistory.erase(fpsHistory.begin()); frameTimeHistory.erase(frameTimeHistory.begin()); }
    ...
    public:
        ...
        bool stopProfiling;
        int frameOffset;
        ProfilerTimeline cpuGraph;
        ProfilerTimeline gpuGraph;
        int frameWidth;
        int frameSpacing;
        bool useColoredLegendText;
